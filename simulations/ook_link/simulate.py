...
def decode(mic_values, sim_dt, n_bits, bit_dur, prop_delay):
    ...
    The threshold is midway between the strongest and weakest bit; this
    works whenever the message contains both 0s and 1s."""
    samples = np.asarray(mic_values, dtype=np.float32)
    spb = int(round(bit_dur / sim_dt))
    delay = int(round(prop_delay / sim_dt))
    rms = np.array([
        float(np.sqrt(np.mean(samples[delay + i * spb : delay + (i + 1) * spb] ** 2)))
...
def run(args, out):
    ...
    sim = AcousticFDTD(
        ...
        sources=[Source(pos=TX, waveform=ook_square(FREQ, MESSAGE, BIT_DURATION, AMPLITUDE))],
    ...
    out.finish(dt=sim.dt * args.capture_every, dx=DX, c=sim.c, channels=(mic,))
