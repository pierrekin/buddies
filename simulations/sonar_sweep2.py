"""Sonar v2: a rigid backstop wall behind the block, and ranging on the
LOUDEST return per angle instead of the first. The array fires one
beamformed ping per angle, beamforms the same angle on receive, and the
strongest arrival in each listen window sets that angle's range. Arrows
are colored by loudness."""
