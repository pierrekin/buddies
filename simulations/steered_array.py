...
DX = WAVELENGTH / 10  # 10 cells per wavelength
...
sim = AcousticFDTD(
    ...
    DX,
    c=C,
    # Coarse playback is handled by CAPTURE_EVERY; no need for a fine dt.
    cfl=0.95,
    sources=array(
...
capture.save(
    OUT, capture.Capture(frames=frames, dt=sim.dt * CAPTURE_EVERY, dx=DX, c=sim.c)
)
