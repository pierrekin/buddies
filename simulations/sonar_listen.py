...
The sweep series (sonar_sweep*) steers the *transmit* beam, firing one
focused ping per look angle -- 31 pings for a 31-angle fan. This series
flips that: fire ONE omnidirectional pulse, record every array element,
and form the look direction afterward by delay-and-summing the
recordings. The delays are identical to the sweep's transmit focusing
(receive reciprocity), so all the machinery carries over; the only real
change is that one recording can be beamformed to any direction we like.

v1 is the smallest thing that works: a single hardcoded look angle aimed
at one target, producing one range vector. The tradeoff to watch is SNR
-- an omni ping spreads its energy over the whole scene instead of
concentrating it at the target, so returns are weaker than the focused
sweep and the source amplitude is turned up to compensate."""
...
# copied to the host once at the end (see simargs --gpu).
