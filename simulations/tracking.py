"""A source that hops between positions, emitting one pulse from each,
while a microphone array re-estimates its bearing after every pulse. The
final position is nearly endfire to the array, where bearing estimates
degrade.
...
HOP_STEPS = 600  # per position: max mic distance ~0.61 m = ~365 steps, plus margin
DETECT_THRESHOLD = 0.2  # of the window's peak |p|
OUT = "captures/tracking.npz"
...
def arrival(envelope):
    """Step where the arrival that contains the window's peak rises through
    the threshold. Searching backward from the peak ignores residual energy
    from earlier pulses at the start of the window."""
