...
It decimates frames in time, quantizes the float32 pressure field to uint8
normalized to a percentile level (so the decaying wavefront stays visible
rather than being swamped by the source peak), and copies channels and the
overlay through unchanged. The level the viewer used to recompute on every
open is baked in here once.
...
def process(master, args, out):
    src = master.frames
    keep = range(0, len(src), args.decimate)
    indices = list(keep)
    dst = out.open((len(indices), *src.shape[1:]), dtype=np.uint8)

    lim = compute_level(src, args.percentile) or 1.0
    # Map [-lim, lim] Pa onto [0, 255] (128 = zero pressure = white center of
    # the diverging map); values beyond the level saturate.
    scale = 127.5 / lim
    with tqdm(total=len(indices), unit="frame") as bar:
        for start in range(0, len(indices), CHUNK):
            block = indices[start : start + CHUNK]
            chunk = np.asarray(src[block], dtype=np.float32)
            dst[start : start + len(block)] = np.clip(chunk * scale + 127.5, 0, 255).astype(
                np.uint8
            )
